...
const KEY = "outline";
const PKG = "@fswap/mcp-outline@latest";
...
test("Claude Desktop: merges into existing config, keeps other keys, backs up once", () => {
  ...
  assert.equal(installClient("claude-desktop", { OUTLINE_API_TOKEN: "ol_api_..." }, env).status, "added");
  assert.deepEqual(JSON.parse(read(file)), {
    globalShortcut: "Cmd+Space",
    mcpServers: { other: { command: "x" }, [KEY]: entry({ OUTLINE_API_TOKEN: "ol_api_..." }) },
  });
...
test("Codex: replaces existing sections and leaves the rest of config.toml alone", () => {
  ...
  assert.equal(installClient("codex", { OUTLINE_URL: "https://app.getoutline.com" }, env).status, "replaced");
  assert.equal(
    read(file),
    [
      'model = "gpt-5"',
...
test("Claude Code: replaces through the claude CLI at user scope", () => {
  ...
  const result = installClient("claude-code", { OUTLINE_URL: "https://app.getoutline.com" }, env);
  assert.deepEqual(result, { status: "replaced", location: "user scope via claude mcp" });
  assert.deepEqual(calls, [
    `claude mcp get ${KEY}`,
    `claude mcp get ${KEY}`,
    `claude mcp remove ${KEY} -s user`,
    `claude mcp add ${KEY} -s user -e OUTLINE_URL=https://app.getoutline.com -- npx -y ${PKG}`,
  ]);
});
